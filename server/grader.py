...
def grade_query(ai_query: str, task: dict, db_connection):
    ...
    try:
        ...
        return {"score": 0.0, "feedback": f"SQL Error: {str(e)}"}
    ...
    if required_tables:
        tables_used = sum(1 for t in required_tables if t.lower() in query_lower)
        total_score += (tables_used / len(required_tables)) * (max_score * 0.20)
        if tables_used < len(required_tables): feedback_msgs.append("Missing tables")
    else:
        total_score += (max_score * 0.20)

    required_keywords = task.get("required_keywords", [])
    if required_keywords:
        keywords_used = sum(1 for k in required_keywords if k.lower() in query_lower)
        total_score += (keywords_used / len(required_keywords)) * (max_score * 0.10)
        if keywords_used < len(required_keywords): feedback_msgs.append("Missing keywords")
    else:
